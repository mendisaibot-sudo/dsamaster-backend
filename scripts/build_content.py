#!/usr/bin/env python3
"""Build content_data.py with all 30 lessons."""

OUT = "/home/mailstonandakumar/.openclaw/workspace/dsamaster-backend/scripts/content_data.py"
...
def fmt_lesson(f, title, slug, diff, mins, secs, codes, exes):
    w(f, f'    ("{title}", "{slug}", "{diff}", {mins}, [')
    for h, p in secs:
        w(f, f'        ({q(h)}, {q(p)}),')
    w(f, '    ], [')
    for lang, code, expl, out in codes:
        w(f, f'        ("{lang}", {q(code)}, {q(expl)}, {q(out)}),')
    w(f, '    ], [')
    for t, question, opts, ans in exes:
        if opts:
            w(f, f'        ("{t}", {q(question)}, {opts}, {q(ans)}),')
        else:
            w(f, f'        ("{t}", {q(question)}, None, {q(ans)}),')
    w(f, '    ]),')

with open(OUT, 'w') as f:
    w(f, '"""Content data for DSAMaster content platform."""')
    w(f, '')
    w(f, '# Each lesson tuple: (title, slug, difficulty, minutes, sections, code_examples, exercises)')
    w(f, '# sections = [(heading, paragraph), ...]')
    w(f, '# code_examples = [(language, code, explanation, output), ...]')
    w(f, '# exercises = [(type, question, options_list, correct_answer), ...]')
    w(f, '')
    
    # ── PYTHON BASICS ──
    w(f, 'PYTHON_BASICS_OUTLINE = [')
    
    fmt_lesson(f, "Variables and Types", "variables-and-types", "beginner", 15,
    ...
    fmt_lesson(f, "Operators and Expressions", "operators-and-expressions", "beginner", 15,
    ...
    fmt_lesson(f, "Control Flow - if/else/elif", "control-flow-if-else", "beginner", 20,
    ...
    fmt_lesson(f, "for Loops", "for-loops", "beginner", 15,
    ...
    fmt_lesson(f, "while Loops", "while-loops", "beginner", 15,
    ...
    fmt_lesson(f, "Functions", "functions", "beginner", 20,
    ...
    fmt_lesson(f, "Strings", "strings", "beginner", 15,
    ...
    fmt_lesson(f, "Lists", "lists", "beginner", 20,
